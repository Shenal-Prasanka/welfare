<div class="modal fade" id="ModalWelfareMembershipCreate" tabindex="-1" role="dialog" aria-labelledby="ModalMembershipTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document"> {{-- Changed to modal-lg for more fields --}}
        <div class="modal-content">
            <div class="modal-header bg-dark">
                <h5 class="m-0 font-weight-bold">{{ __('Add New Welfare-Membership') }}</h5>
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('welfarememberships.store') }}" enctype="multipart/form-data" novalidate>
                    @csrf
            ...
            <div class="modal-footer">
                <button type="submit" class="btn btn-primary">{{ __('Add Welfare Membership') }}</button>
...
@if ($errors->any())
    ...
    @php
        $welfaremembership_fields = ['name', 'email', 'mobile', 'address', 'membership_date', 'army_id', 'regiment_no', 'nic', 'active'];
        $has_welfaremembership_errors = false;
        foreach ($errors->keys() as $key) {
            if (in_array($key, $welfaremembership_fields)) {
                $has_welfaremembership_errors = true;
                break;
            }
        }
    @endphp

    @if($has_welfaremembership_errors)
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Ensure Bootstrap is loaded if you rely on its Modal class, 
                // otherwise fall back to jQuery if available.
                const modalElement = document.getElementById('ModalWelfareMembershipCreate');
                if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                    const modal = new bootstrap.Modal(modalElement);
                    modal.show();
                } else if (typeof $ !== 'undefined') {
                    $('#ModalWelfareMembershipCreate').modal('show');
                }
            });
        </script>
    @endif
